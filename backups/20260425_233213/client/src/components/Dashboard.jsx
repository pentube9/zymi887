...
function Dashboard({ user, onLogout }) {
  ...
  useEffect(() => {
   const fetchUnread = async () => {
     try {
       const res = await fetch(`http://localhost:3001/api/unread/${user.id}`, {
         headers: getAuthHeaders()
       });
       const data = await res.json();
       const counts = {};
   ...
   useEffect(() => {
     if (!selectedUser || !socket) return;
      ...
      const fetchMessages = async () => {
        try {
          const res = await fetch(`http://localhost:3001/api/messages/${user.id}/${selectedUser.id}`, {
            headers: getAuthHeaders()
          });
          if (!res.ok) return;
          const data = await res.json();
  ...
  const saveEdit = async (msg) => {
    ...
    try {
      const token = localStorage.getItem('zymi_token');
      const res = await fetch(`http://localhost:3001/api/messages/${msg.id}/edit`, {
        method: 'PUT',
        headers: {
  ...
  const handleTyping = (e) => {
   ...
   const handleMessageSearch = async (e) => {
     ...
     if (e.target.value.trim().length > 0) {
       try {
         const res = await fetch(`http://localhost:3001/api/messages/search/${user.id}?q=${encodeURIComponent(e.target.value)}`, {
           headers: getAuthHeaders()
         });
         const data = await res.json();
  ...
  const handleAcceptCall = async () => {
    ...
    soundService.stopCallRingtone();
    setSelectedUser(users.find(u => u.id === incomingCall.from));
  ...
  useEffect(() => {
    if (connectionStatus === 'disconnected' || connectionStatus === 'offline') {
      ...
      setCallError(prev => prev === 'Connection lost. Please check your internet.' ? '' : prev);
    }
  }, [connectionStatus]);
            ...
            <div className="chat-header">
              ...
              {callStatus === 'idle' && (
                <div className="call-buttons">
                  <button className="call-btn audio-btn" onClick={() => startCall(selectedUser.id, 'audio')}>
                    <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
                      <path d="M22 16.92v3a2 2 0 0 1-2.18 2 19.79 19.79 0 0 1-8.63-3.07 19.5 19.5 0 0 1-6-6 19.79 19.79 0 0 1-3.07-8.67A2 2 0 0 1 4.11 2h3a2 2 0 0 1 2 1.72 12.84 12.84 0 0 0 .7 2.81 2 2 0 0 1-.45 2.11L8.09 9.91a16 16 0 0 0 6 6l1.27-1.27a2 2 0 0 1 2.11-.45 12.84 12.84 0 0 0 2.81.7A2 2 0 0 1 22 16.92z"/>
                    </svg>
                    Audio Call
                  </button>
                  <button className="call-btn video-btn" onClick={() => startCall(selectedUser.id, 'video')}>
                    <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
...
{(callStatus === 'connected' || callStatus === 'calling') && (
            ...
            <div className="messages-area">
              {messages.map((msg, i) => {
                ...
                return (
                  <div key={msg.id || i} className={`message ${isOwn ? 'sent' : 'received'}`}>
